...
$(document).ready(function(){
	...
	// console.log(final_category_id);
	// console.log(final_subcategory_id);
	// console.log(final_location_id);
	var id=event.target.id;
		// console.log(id);
	var arr=id.split('e');
	var part_id=arr[1] ;
		 ...
		 if(data.status==1)
		    {
		    	...
		    	$.each(data.data, function(key, value) {
		    		...
		    		if(parts>1){
			    		...
			    		$("#part"+parts+' .arr-box').append('<div class="table-item items '+value.class+' adddisable" id="course'+value.course_id+'" value="'+value.course_id+'"><input type="hidden" id="hidden_course_date" value="'+value.course_date+'" ><input type="hidden" id="hidden_slot" value="'+value.slots+'" ><h5>'+value.course_category+' <?php echo $translation['_course_part_']?>  '+value.course_no+'  '+value.course_daytime+'</h5><p><b>'+value.location_name+'</b></p><p><b>'+value.date_time+'</b></p><p><b>'+value.slots+' <?php echo $translation['_available_']?></b></p></div>');
			    	}else{

			    		$("#part"+parts+' .arr-box').append('<div class="table-item items '+value.class+'" id="course'+value.course_id+'" value="'+value.course_id+'"><input type="hidden" id="hidden_course_date" value="'+value.course_date+'" ><input type="hidden" id="hidden_slot" value="'+value.slots+'" ><h5>'+value.course_category+' <?php echo $translation['_course_part_']?>  '+value.course_no+'  '+value.course_daytime+'</h5><p><b>'+value.location_name+'</b></p><p><b>'+value.date_time+'</b></p><p><b>'+value.slots+' <?php echo $translation['_available_']?></b></p></div>');
			    	}
		    		//$("#part"+parts+' .arr-box').append('<div class="table-item items '+value.class+'" id="course'+value.course_id+'" value="'+value.course_id+'"><input type="hidden" id="hidden_course_date" value="'+value.course_date+'" ><input type="hidden" id="hidden_slot" value="'+value.slots+'" ><h5>'+value.course_category+' <?php echo $translation['_course_part_']?>  '+value.course_no+'  '+value.course_daytime+'<?php echo $translation['_course2_']?></h5><p><b>'+value.location_name+'</b></p><p><b>'+value.date_time+'</b></p><p><b>'+value.slots+' <?php echo $translation['_available_']?></b></p></div>');
		    	...
		    	$(".items").click(function(){
				    ...
				    var next_part_id = $(this).parent().parent().next().attr("id");
...
$("a#location-done").click(function(){
	...
	// newpage();
	// console.log('user_id'+user_id);
	// console.log('final_subcategory_id'+final_subcategory_id);
	// console.log('final_location_id'+final_location_id);
	// console.log('final_category_id'+final_category_id);
    ...
    $.ajax({
		url : url,
		type: "POST",
		data: {user_id:user_id,category_id:final_category_id,sub_category_id:final_subcategory_id,location_id:final_location_id,type:'get_courses',page_no:1,per_page:6},
		dataType: "JSON",
		success: function(data)
		{
		    ...
		    if(data.status==1)
		    {
		    	console.log(data.courseStartTime);
				...
				{
					var a= i-1;
                    // console.log(data.part_name[a]['name']);
                    if(data.total_course[a]['course_count']>6){
				...
				// $.each(data.total_course, function(key, value) {
				// 		    		// i++;
				// 		    		// if(i>2){
				// 		    		console.log(value);
				// 		    		if(value.course_count<8){
				// 		    			$(".see_m_button").hide();
				// 		    		}else{
				// 		    			$(".see_m_button").show();
				// 		    		}
		    	...
		    	$.each(data.data, function(key, value) {
		    		$.each(value, function(key, value) {
		    	     console.log(value.course_start_time+' course_start_time');
		    		if(value.part==""){
		    			var parts=value.sub_part_no;
		    		}
		    		else{
		    		...
		    		if(parts>1){
		    		$("#part"+parts+' .arr-box').append('<div class="table-item '+value.class+' adddisable" id="course'+value.course_id+'" value="'+value.course_id+'"><input type="hidden" id="hidden_course_date" value="'+value.course_date+'" ><input type="hidden" id="hidden_slot" value="'+value.slots+'" ><h5>'+value.course_category+'  <?php echo $translation['_course_part_']?>  '+value.course_no+'  '+value.course_daytime+'<?php echo $translation['_course2_']?> </h5><p><b>'+value.location_name+'</b></p><p><b>'+value.date_time+'</b></p><p><b>'+value.slots+' <?php echo $translation['_available_']?></b></p></div>');

		    		}else{

		    		$("#part"+parts+' .arr-box').append('<div class="table-item '+value.class+'" id="course'+value.course_id+'" value="'+value.course_id+'"><input type="hidden" id="hidden_course_date" value="'+value.course_date+'" ><input type="hidden" id="hidden_slot" value="'+value.slots+'" ><h5>'+value.course_category+'  <?php echo $translation['_course_part_']?>  '+value.course_no+'  '+value.course_daytime+'<?php echo $translation['_course2_']?> </h5><p><b>'+value.location_name+'</b></p><p><b>'+value.date_time+'</b></p><p><b>'+value.slots+' <?php echo $translation['_available_']?></b></p></div>');
		    		}
		    		

		    	});
		    		});
		    		// var course_arr = $('#final_course_id').val();
					// 	    		var course =Object.values(course_arr);
					// console.log('course_id'+Object.values(course_arr));
					// console.log(course.length);
					// var co = $('#final_course_id').val();
					//  size = document.getElementById("final_course_id").value;
					// console.log('co'+size);
					// 		for (var i = 0; i <=co.length; i++) {
					// var inp=co[i];
					//    console.log("inp"+inp);
					// //    // inp.pop();
					// //    var c =Object.values(course_arr);
					//    co.pop(inp);
					// //    console.log("obj"+Object.values(course_arr));
					// }
					// console.log('course_id1111p'+course_arr);
    ...
    $.ajax({
		...
		success: function(data)
		{
		    if(data.status==1)
		    {
		    	...
		    	$.each(data.data, function(key, value) {
		    		$('#review_div3 h5').after('<p><i class="fa fa-certificate" aria-hidden="true"></i> <b>'+value.course_detail+'<?php echo $translation['_course_part_']?>  '+value.course_no+'  '+value.course_day+'<?php echo $translation['_course2_']?></b></p><p>'+value.date_time+'</p>');
		    	});
