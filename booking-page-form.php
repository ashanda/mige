...
if($_SESSION['language']=='en'){
	$translations['_phone_number_valid_'] = ['Please enter Valid 10 Digit Phone Number.'] ;
}else{
	$translations['_phone_number_valid_'] = ['Bitte geben Sie eine gültige 10-stellige Telefonnummer ein.'] ;
}
if($_SESSION['transaction_id']){
	
	echo "<script type='text/javascript'>window.location.href='paymentStatus/?datatransTrxId=".$_SESSION['transaction_id']."';</script>";
}
      ...
      <div class="row">
  		<div class="col-md-12 col-sm-12 no-padding animatedParent">
        	...
        	<div class="contact-text-wrapper" style="padding:90px; padding-bottom:0px;" id="rgster">
				...
				<form id="booking-form" action="<?=$config['base_url']?>/GetPhpData.php" method="POST">
					<div id="booking-page-fl">
							...
							<div class="booking-details">
								<div class="book-det for0-m">
									<div class="form-group">
										...
										<p><input class="form-control trimleftspace" type="text" name="firstname" id="firstname" required></p>
									</div>
									<div class="form-group">
										<h5><?php echo $translation['_Surname_']?> <span>*</span> </h5>
										<p><input class="form-control trimleftspace" type="text" name="lastname" id="lastname" required></p>
									</div>
									<div class="form-group">
										<h5><?php echo $translation['_Firmenname_']?> </h5>
										<p><input class="form-control" type="text" name="company_name" id="company_name"></p>
									</div>
									<div class="form-group">
										<h5><?php echo $translation['_Country_']?><span>*</span></h5>
										<p><input class="form-control trimleftspace" type="text" placeholder="Switzerland" name="country" disabled id="country"></p>
									</div>
									<div class="form-group">
										<h5><?php echo $translation['_Postleitzahl_']?> <span>*</span> </h5>
										<p><input class="form-control trimleftspace" type="text" placeholder="" name="postcode" id="postcode" required></p>
									</div>
									<div class="form-group">
										<h5><?php echo $translation['_Phone_title_']?> <span>*</span> </h5>
										<p><input class="form-control trimleftspace" type="text" placeholder="" id="phone" name="phone" required></p>
									</div>
									<div class="form-group">
										<h5><?php echo $translation['_Street_']?><span>*</span> </h5>
										<p><input class="form-control trimleftspace" id="street" type="text" placeholder="<?php echo $translation['_Stree_house_number_']?>" name="street" required></p>
									</div>
									<div class="form-group">
										<h5><?php echo $translation['_city_']?><span>*</span> </h5>
										<p><input class="form-control trimleftspace" id="city" type="text" placeholder="<?php echo $translation['_city_']?>" name="city" required></p>
									</div>
									<div class="form-group">
										<h5><?php echo $translation['_Email_Address_Title_']?> <span>*</span> </h5>
										<p><input class="form-control trimleftspace" type="email" name="email" id="email" required autocomplete="off"></p>
										<span id="error"></span>
									</div>
									<div class="form-group">
										<h5><?php echo $translation['_Create_account_passsword_']?> <span>*</span> </h5>
										<p><input class="form-control trimleftspace" autocomplete="off" type="password" name="password" id="password" required></p>
									</div>
								</div>
						...
						<div class="sub-details-cart">
							...
							<div class="twint-options">
								...
								<!--  <div class="twint">
									<div class="form-group in-r"><input type="radio" name="payment_type" value="twint" class="sel-radio" id="twintButton">  <b></b><img class="twint-img" src="images/twint_logo_q.svg"></div>
								...
								<div class="inpu-ch">
									<div class="form-group"><input type="checkbox" required><?php echo $translation['_I_have_the_']?> <a href="<?=$config['base_url']?>/privacy.php"><?php echo $translation['_terms_and_conditions_']?></a> <?php echo $translation['_lesen_']?></div>
								</div>
...
// }
    ...
    {
	    $.ajax({
	        url : '<?=$config['base_url']?>/GetPhpData.php',
	        type: "POST",
	...
	$.ajax({
		...
		success: function(data)
		{
		    ...
		    if(data.status==1)
		    {
				...
				//	console.log(value);
		    		$('tbody#courses').append('<tr><td>'+value.course_detail+' <?php echo $translation['_course_part_']?>  '+value.course_no+'  '+value.course_day+' | '+value.c_date+' @ '+value.c_time+'-'+value.c_end_time+' | '+value.place+'  × 1</td><td>CHF '+value.price+'</td></tr>');
		...
		$.ajax({
			...
			success: function(data)
			{
			    ...
			    else
			    {
				    ...
				    {
				    	alert('<?=$translations['_phone_number_valid_'][0]?>');
				    	$('#phone').focus(); 
				    }
				    else 
				    {
...
$("#twintButton").click(function() {
	// alert('fsdf');
	var amount=$('#totalAmountTwint').val();
	var url = "sendPaymentWithTwint.php";
	$.ajax({
	        url : url,
	        type: "POST",
	        data: {'amount':amount,'url':"web"},
	        dataType: "JSON",
	        success: function(data)
	        {
	           // console.log(data.transactionID);
	            $('#transaction_id').val(data.transactionID);
	           // var url='https://pay.datatrans.com/v1/start/'+data.transactionID;
...
/*$('input.trimleftspace').on('change',function(e){
